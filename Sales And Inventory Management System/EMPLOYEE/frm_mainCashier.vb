...
Public Class frm_mainCashier
    ...
    Private Sub dgv_supplierList_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles dgv_supplierList.CellContentClick
        ...
        Try
            ...
            If colNamepro = "EditSup" Then
                ...
                txt_supplierName.Text = dt.Cells(1).Value.ToString()
                txt_productname.Text = dt.Cells(2).Value.ToString()
                txt_address.Text = dt.Cells(3).Value.ToString()
                txt_supplierNumber.Text = dt.Cells(4).Value.ToString()
                txt_email.Text = dt.Cells(5).Value.ToString()
    ...
    Private Sub dgv_returnList_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles dgv_returnList.CellContentClick
        Dim colNamepro As String = dgv_returnList.Columns(e.ColumnIndex).Name
        Try
            connection = "Server=DESKTOP-B55CCOO\SQLEXPRESS01;Database=SalesAndInventory;Trusted_Connection=True;"
            connstring = connection
            myconnection.ConnectionString = connstring
            myconnection.Open()
            colNamepro = "DeleteReturn"
            ...
            If password = adminUsername Then
                Dim dt1 As DataGridViewRow = dgv_returnList.SelectedRows(0)
                dtp_return.Text = dt1.Cells(1).Value.ToString()
                txt_creturn.Text = dt1.Cells(2).Value.ToString()
                txt_conreturn.Text = dt1.Cells(3).Value.ToString()
                cbo_returnpro.Text = dt1.Cells(4).Value.ToString()
                txt_qtyreturn.Text = dt1.Cells(5).Value.ToString()
                txt_issue.Text = dt1.Cells(6).Value.ToString()
                cbo_action.Text = dt1.Cells(7).Value.ToString()

                Dim query As String = "DELETE from tbl_return Where [Cname] = '" & txt_creturn.Text & "'"
                Using cmd As New SqlCommand(query, myconnection)
                    cmd.ExecuteNonQuery()
                End Using
                MsgBox("Success Delete Product !", vbInformation, "Success")
                dgv_returnList.Rows.RemoveAt(dgv_returnList.SelectedRows(0).Index)
                txt_creturn.Focus()
                clearReturn()
            Else
    ...
    Private Sub txt_conreturn_Validating(sender As Object, e As System.ComponentModel.CancelEventArgs) Handles txt_conreturn.Validating

        Dim minContactLength As Integer = 11 ' Minimum length of the contact number
        Dim maxContactLength As Integer = 12 ' Maximum length of the contact number

        If txt_supplierNumber.TextLength > 0 AndAlso (txt_conreturn.TextLength < minContactLength OrElse txt_conreturn.TextLength > maxContactLength) Then
            MessageBox.Show("Contact number should be between " & minContactLength & " and " & maxContactLength & " digits.", "Invalid Contact", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            e.Cancel = True
            txt_supplierNumber.Clear()
        End If
    ...
    Private Sub txt_searchret_TextChanged(sender As Object, e As EventArgs) Handles txt_searchret.TextChanged
        Try
            ...
            Dim query As String
            query = "Select RDate, Cname, Contact, Pname, Qty, Issue, Acttaken From tbl_return Where [Cname] like '%" & txt_searchret.Text & "%' or [Pname] like '%" & txt_searchret.Text & "%' or [Acttaken] like '%" & txt_searchret.Text & "%'"
            Dim dt As New DataTable
            Dim command As SqlCommand = New SqlCommand(query, myconnection)
            Dim da As New SqlDataAdapter
            da.SelectCommand = command
